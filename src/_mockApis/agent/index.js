...
services.onGet('/api/agent/list').reply(200, { agents });

services.onPost('/api/agent').reply((request) => {
    try {
...
services.onPatch('/api/agent').reply((request) => {
    try {
        ...
        const objIndex = agents.findIndex((obj) => obj.id === id);

        agents[objIndex] = user;
...
services.onPost('/api/agent/delete').reply((request) => {
    try {
