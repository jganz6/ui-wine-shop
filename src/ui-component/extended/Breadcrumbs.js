...
const Breadcrumbs = ({ card, divider, icon, icons, maxItems, navigation, rightAlign, separator, title, titleBottom, ...others }) => {
    ...
    const getCollapse = (menu) => {
        if (menu.children) {
            menu.children.filter((collapse) => {
                if (collapse.type && (collapse.type === 'collapse' || collapse.type === 'subtance')) {
                    getCollapse(collapse);
                } else if (collapse.type && collapse.type === 'item') {
                    const len = document.location.pathname.split('/').length;
                    if (
                        document.location.pathname
                            .split('/')
                            .slice(0, len - Object.keys(urlParams).length)
                            .join('/') ===
    ...
    // collapse item
    if (main && (main.type === 'collapse' || main.type === 'subtance')) {
                            ...
                            {title && !titleBottom && (
                                <Grid item>
                                    <Typography variant="h3" sx={{ fontWeight: 500 }}>
                                        {param ? `${item.title} : ${param}` : item.title}
                                    </Typography>
                                </Grid>
                            )}
                            <Grid item>
