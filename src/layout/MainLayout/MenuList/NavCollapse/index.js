...
const NavCollapse = ({ menu, level, parentId }) => {
    ...
    const checkOpenForParent = (child, id) => {
        child.forEach((item) => {
            if (
                ...
                pathname
                    .split('/')
                    .slice(0, pathname.split('/').length - Object.keys(urlParams).length)
                    .join('/')
            ) {
                setOpen(true);
                setSelected(id);
    ...
    useEffect(() => {
        ...
        childrens.forEach((item) => {
            ...
            if (pathname && pathname.includes('receive-list')) {
                if (item.url && item.url.includes('receive-list')) {
                    setSelected('inventory');
                    setOpen(true);
                }
            }
            if ((pathname && pathname.includes('purchase-order')) || pathname.includes('master-item')) {
                if ((item.url && item.url.includes('purchase-order')) || item.url.includes('master-item')) {
                    setSelected('inventory-management');
                    setOpen(true);
                }
            }
            if (
                ...
                <>
                    ...
                    >
                        {menuIcon && (
                            <ListItemIcon
                                sx={{
                                    mr: isSelected && menu?.icon ? 1 : 'inherit',
